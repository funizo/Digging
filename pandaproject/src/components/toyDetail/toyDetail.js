...
function ToyDetail() {
  ...
  const handleAddToWishlist = async () => {
    ...
    try {
      ...
      formData.append('title', toyData.toyTitle);
      formData.append('content', toyData.toyContent);
      formData.append('image', toyData.toyImg);
  ...
  const handleDelete = async() => {
      try {
        const response = await fetch('http://localhost:8080/category/toy/toydetail', {
          method: 'DELETE',
          headers: {
  ...
  const handleEdit = () => {
      navigate(`/toyedit/${toyData.id}`, {state:{toyData}});
  }
  ...
  return (
    <div>
      ...
      <div className="book_container">
        <img src={toyData.toyImg} alt={toyData.title} />
        <div className="title_deco">
          <h2>제목: {toyData.toyTitle}</h2>
          <p>내용: {toyData.toyContent}</p>
          <p>즉시 구매가: {toyData.price}</p>
        </div>
      </div>
      ...
      <div className="wishlist">
        ...
        <button className="button_wishlist" onClick={handleAddToWishlist} style={
          { display: userInfo && (userInfo.id === toyData.id || toyData.id === '65703c972d7eba2e853faa06') ? 'block' : 'none' }
          }>
            {isAddedToWishlist ? "찜" : "관심 상품으로 찜하기"}
        </button>
        <button className="button_wishlist" onClick={handleDelete} style={
            { display: userInfo && (userInfo.id === toyData.id || userInfo.id === '65703c972d7eba2e853faa06') ? 'block' : 'none' }
          }>
            삭제하기
        </button>
